...
const modalFields: FieldDef[] = [
  { name: 'manufacturerName', label: 'نام سازنده', type: 'string', required: true, minLength: 2, maxLength: 200, placeholder: 'نام سازنده را وارد کنید' },
...
export default function ManufacturersPage() {
  ...
  const handleSave = async (values: Record<string, unknown>) => {
    ...
    try {
      if (modalMode === 'create') {
        await apiClient.post('/v1/api/zootag/admin/manufacturers', values);
      } else {
        await apiClient.put(`/v1/api/zootag/admin/manufacturers/${selected!.id}`, values);
      }
      setModalOpen(false);
      setRefreshKey((k) => k + 1);
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`/v1/api/zootag/admin/manufacturers/${deleteTarget.id}`);
  ...
  return (
    <div>
      ...
      <CrudModal
        open={modalOpen}
        mode={modalMode}
        title={modalMode === 'create' ? 'افزودن سازنده' : 'ویرایش سازنده'}
        fields={modalFields}
